...
export function drawScuiDebug(ctx: InteractiveCanvasFrameContext, root: RootComponent) {
    ...
    const componentName = className.endsWith("Component")
    ...
    textWithBackground(ctx, position, `${componentName}: ${width} x ${height}`, {
        background: {
            fill: "#8a5598",
            thickness: 1,
            colour: "#f6d1ff"
