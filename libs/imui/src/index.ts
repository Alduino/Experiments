...
export type RenderOptions = FillOptions | OutlineOptions | RenderDisabledOptions;
...
export function lines(ctx: CanvasFrameContext, points: readonly Vector2[], opts?: RenderOptions) {
    if (points.length === 0) {
        return;
    }
    ...
    ctx.renderer.moveTo(start.x, start.y);
